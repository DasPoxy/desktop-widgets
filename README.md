...
| **Abyss Warden** | `AbyssWardenWidget.qml` + `AbyssEye.qml`, `get-abyss-watch.sh` | An anime eye that only appears while the screen is being recorded: Omarchy's recorder, wf-recorder/wl-screenrec, or any portal capture (OBS, Discord, browser screen share). Awake, it follows the mouse, glances at windows whose title changes, darts around and stares at the viewer, and blinks. Right-click: Free-Floating mode (click-through, drifts around the screen), card background, 15s preview, and 7 eye themes or the system theme. |
